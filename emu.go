...
func (s Section) Has(addr uint64) bool {
	if s.Addr         > addr { return false }
	if s.Addr+s.Size  < addr { return false }
	return true
}
...
func Boot(dir string) (e *Emu, err error) {
	...
	checkerr  := func() {if err != nil {panic(err)}}
	// TODO: not always 32 bit
	base, err := read64(dir+"/opthdr32/addr/baseimage"); checkerr()
	emu = new(Emu)
	emu.Base = base
	...
	for _,v := range sections(dir) {
		...
		addr, err = read64(subdir + "/offset"); checkerr()
		size, err = read64(subdir + "/size");   checkerr()
		data, err = read(subdir + "/data/raw"); checkerr()
